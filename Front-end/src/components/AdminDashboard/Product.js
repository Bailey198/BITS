...
const Product = () => {
    ...
    const requestDeleteApi = () => {
        if(deleteType === 'single'){
            ...
            requestApi(`/products/${deleteItem}`, 'DELETE', []).then(response => {
                setPopUp(false)
                setRefresh(Date.now())
            ...
            dispatch(actions.controlLoading(true))
            requestApi(`/products/multiple?ids=${selectedRows.toString()}`, 'DELETE', []).then(response => {
                setPopUp(false)
                setSelectedRows([])
                setRefresh(Date.now())
                dispatch(actions.controlLoading(false))
            ...
            <div className={popUp ? 'brightness-50' : null}>
                <div className="bg-white px-4 pt-3 pb-4 rounded-sm border border-gray-200 flex-1">
                    ...
                    <div className='flex text-black justify-between'>
                        <h2 className='me-2 h3'>Total Products: {totalProduct}</h2>
